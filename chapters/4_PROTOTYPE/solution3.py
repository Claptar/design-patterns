...
class DashboardCustomCloneTests(unittest.TestCase):
    def make_dashboard(self) -> Dashboard:
        source = DataSource(name="warehouse", connection_string="postgresql://analytics-db")
        dashboard = Dashboard(
...
Explanation
-----------
This clone() method does not blindly deep-copy the entire dashboard. It applies
custom clone rules:

- widgets are copied because each dashboard should be able to edit widget titles
  and chart choices independently.
- filters are copied because each dashboard should have independent filter values.
- data_source is shared because it represents shared configuration.
- cache is reset because cached runtime results should not leak into a fresh clone.

This is closer to real Prototype code than simply calling deepcopy(self).

Why validate unknown fields?
----------------------------
Without validation, this call would silently create a new attribute:

    dashboard.clone(owenr="Alice")

That typo would be hard to notice. Raising AttributeError makes mistakes obvious.

Pitfall
-------
There is no universal clone rule. You must decide which parts are copied, shared,
or reset based on the meaning of the object.
"""
